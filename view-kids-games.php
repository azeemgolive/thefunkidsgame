...
$games_detail= getGameBySeo($seo);
$detail_game=  mysql_fetch_array($games_detail);
?>
<!doctype html>
<html lang="en">
<head>
...
<script type="text/ javascript">
  stLight.options({
    ...
    hashAddressBar: false
        ...
        <meta property="og:image" content="http://www.thefunkids.com/games/game_images/<?php echo $share_kids_img=$detail_game['game_image']; ?>" />
...
<section class="sec1">
  <div class="container">
    <div class="row">
        <?php
    //$game_id=$_GET['id'];
     $game_detail= getGameBySeo($seo);
    if($game_detail>0)
    {
        while($game= mysql_fetch_array($game_detail))
        {
            $game_id=$game['game_id'];
    ?>
      <div class="col-lg-12"> <object width="100%" height="500" data="games/game_files/<?php echo $game['game_file']; ?>"></object></div>
     <?php
        }
    }
    ?>
	...
	<?php
 $title=urlencode($detail_game['game_title']);
 //$url=urlencode('http://www.facebook.com/yourfanpage');
 $summary=urlencode($detail_game['meta_tag_description']);
 $image=$detail_game['game_image'];
 ?>
